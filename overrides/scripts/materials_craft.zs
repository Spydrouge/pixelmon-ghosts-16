...
for i in allLavaMetals{
	...
	if(i == "silicon_bronze")
	{
		bucket = <item:tconstruct:molten_tinkers_bronze_bucket>;
	}
	else if(i in tinkersArcheryMetals)
	{
		bucket = <item:tinkersarchery:molten_${i}_bucket>;
	}
	else if(i == "fairy" ||i == "starmetal" )
	{
		bucket = <item:materialis:molten_${i}_bucket>;
	}
	else
	{
		bucket = <item:tconstruct:molten_${i}_bucket>;
	}
	...
	if(i != "starmetal")
	{
		craftingTable.addShapeless(
			...
			//	<tagmanager:items>.getTag(loc), 
			//	<item:botania:mana_powder>, 
			//	<item:materialis:molten_quicksilver_bucket>.transform(<item:minecraft:air> * 1)
			//]
